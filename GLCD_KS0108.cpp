...
// GLCD_KS0108::GLCD_KS0108(uint8_t DI, uint8_t RW, uint8_t EN, uint8_t D4, 
// 		uint8_t D5, uint8_t D6, uint8_t D7, uint8_t CS1, uint8_t CS2) {

// 	init(DI, RW, EN, D4, D5, D6, D7, 0, 0, 0, 0, CS1, CS2, BIT_4);
// }
...
void GLCD_KS0108::init(uint8_t DI, uint8_t RW, uint8_t EN, 
	...
	/* displayBitMod = bitMod;
	dataPinsSize = displayBitMod&BIT_4
					? (sizeof(dPins)/sizeof(uint8_t))/2
					: sizeof(dPins)/sizeof(uint8_t); */
...
void GLCD_KS0108::gotoXY(uint8_t x, uint8_t y) {
	...
	this->lcdCoord.x = x;
	this->lcdCoord.y = y;
	this->lcdCoord.p = y/BIT_8;

	for(uint8_t i=0; i < DISPLAY_WIDTH/CHIP_WIDTH; i++) {
		_select_chip(i);
		_ram_write_command(LCD_PAGE | lcdCoord.p);
	}

	_select_chip(lcdCoord.x/CHIP_WIDTH);
	_ram_write_command(LCD_ADDR | lcdCoord.x % CHIP_WIDTH);
}
...
void GLCD_KS0108::drawHorizontalLine(uint8_t x, uint8_t y, uint8_t length) {
	draw_line(x, y, length, 0);
}

void GLCD_KS0108::drawVerticallyLine(uint8_t x, uint8_t y, uint8_t length) {
...
void GLCD_KS0108::draw_char(uint8_t _char) {
	uint8_t row, pageOffset, rData, fontBit, new_fontBit;
	...
	for(uint8_t col=0; col<FONT_WIDTH; col++) {
		...
		// First byte of each line is the width of the character
		fontBit = pgm_read_byte(&*(*(FONT_8X5 + row) + col));
...
void GLCD_KS0108::__send_bits(uint8_t _val, uint8_t _mod) {
	...
	// if(displayBitMod & BIT_4) {
	//     __write_bits(_val>>4, BIT_4);					// Send higher nibble
	//     __write_bits(_val & LOW_NIBBLE, BIT_4);			// Send lower nibble
	// } else if(displayBitMod & BIT_8) {
	//     __write_bits(_val, BIT_0);
	// }
